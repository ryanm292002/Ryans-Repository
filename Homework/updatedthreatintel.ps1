...
$the_cmd = Read-Host -Prompt "Please enter 'SCP' if you would like to put the file on your remote system or just leave it blank to just save it locally"
 
 if ($the_cmd -eq "SCP") {
 

Set-SCPItem -ComputerName web01-ryan (Get-Credential ryan) -Destination '/home/ryan' -Path './iptables.bash' }
...
Set-SCPItem -ComputerName web01-ryan (Get-Credential ryan) -Destination '/home/ryan' -Path './ciscodeny.bash' }
...
Set-SCPItem -ComputerName web01-ryan (Get-Credential ryan) -Destination '/home/ryan' -Path './windows.bash' }
